...
const StockDetails = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        ws.current = new WebSocket(
          `${import.meta.env.VITE_FLASK_BACKEND_URL.replace("http", "ws")}/ws`,
        );
  ...
  const handleBuy = async (order_t) => {
    ...
    if (
      orderType === "LIMIT" &&
      (!limitPrice || isNaN(limitPrice) || limitPrice <= 0)
    ) {
      setOrderStatus({
        success: false,
    ...
    try {
      ...
      const backendUrl =
        import.meta.env.VITE_FLASK_BACKEND_URL || "http://localhost:8000";
      ...
      if (response.data.order) {
        ...
        if (response.data.order.order_type === "BUY") {
          const cost =
            response.data.order.quantity * response.data.order.target_price;
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      if (stockHolding.Quantity < quantity) {
        throw new Error(
          `Only ${stockHolding.Quantity} shares available to sell`,
        );
      }
      ...
      const orderData = {
        ...
        Type: "SELL",
        Price:
          orderType === "LIMIT" ? parseFloat(limitPrice) : stockData.lastPrice,
  ...
  if (!stockData)
    return <div className="p-4 text-center">No stock data available</div>;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 bg-white shadow-lg rounded-lg">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-baseline">
          ...
          <p className="text-gray-500 text-sm">
            {stockData.indexSymbol || stockData.symbol}
          </p>
        </div>
        ...
        <div className="flex items-baseline mt-2">
          ...
          <span
            className={`text-lg ${isPriceUp ? "text-green-600" : "text-red-600"}`}
          >
            {isPriceUp ? "+" : ""}
      ...
      <div className="mb-6 bg-gray-50 p-4 rounded-lg">
        ...
        {orderStatus && (
          <div
            className={`mb-3 p-2 rounded ${orderStatus.success ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
          >
            {orderStatus.message}
          </div>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="mt-2">
            ...
            <div className="mt-4 text-center">
              ...
              <p className="text-xs text-gray-500">
                {(((lastPrice - low) / (high - low)) * 100).toFixed(1)}% of
                today's range
              </p>
            </div>
          </div>
        ...
        <div className="mt-4 text-center">
          <a
            href="https://growwup-screener-1.onrender.com/"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center rounded-lg bg-blue-600 px-6 py-3 text-white font-semibold shadow-md hover:bg-blue-700 transition duration-200"
          >
            Get More Details About Stock
          </a>
        </div>
          ...
          <div className="bg-gray-50 p-4 rounded md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <p
                  className={`font-medium ${stockData.marketStatus === "Open" ? "text-green-600" : "text-red-600"}`}
                >
                  {stockData.marketStatus}
                </p>
              </div>
